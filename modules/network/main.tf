...
resource "aws_subnet" "main_subnet_ilb_1a" {
  vpc_id = aws_vpc.main_vpc.id
  cidr_block = var.main_subnet_ilb_1a_cidr
  availability_zone = "${var.region}a"

  tags = {
    Name = "${var.domain}-${var.env}-main-ilb-private-1a"
    SubnetType = "Utility"
  }
  lifecycle {
    ignore_changes = [tags]
...
resource "aws_subnet" "main_subnet_ilb_1b" {
  vpc_id = aws_vpc.main_vpc.id
  cidr_block = var.main_subnet_ilb_1b_cidr
  availability_zone = "${var.region}b"

  tags = {
    Name = "${var.domain}-${var.env}-main-ilb-private-1b"
    SubnetType = "Utility"
  }
  lifecycle {
    ignore_changes = [tags]
